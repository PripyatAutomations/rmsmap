...
//
// 2022-11-04 Modified for rmsmap to not care about case while parsing maidenhead
// 2022-11-04 added truncateDecimal and parseDecDegrees for rmsmap
...
function parseDecDegrees(instr) {
   var lat = -127.2, lon = -191.1, exp, ret;

   if ((exp = instr.split(",")) != null) {
      lat = exp[0];
      lon = exp[1];
   }

   if (lat != -127.2 && lon != -191.1) {
      ret = [lat, lon];
      console.log("parseDecDegrees '" + instr + "' => ", ret);
   } else {
      console.log("invalid string passed to parseDecDegrees: " + instr);
      ret = null;
   }
   return ret;
}
...
gridSquareToLatLon = function(grid, obj){
  ...
  if (/^[A-Xa-x][A-Xa-x][0-9][0-9]$/.test(grid)) {
    ...
    lat = lat4(grid.toUpperCase()) + (1.0 / 60.0) * 2.5 * (grid.charCodeAt(5) - aNum + 0.5);
    lon = lon4(grid.toUpperCase()) + (1.0 / 60.0) * 5 * (grid.charCodeAt(4) - aNum + 0.5);
  } else
  ...
  // We only care about 3 decimals, really
  lat = truncateDecimal(lat, 3);
  lon = truncateDecimal(lon, 3);
  return [lat, lon];
};
...
testGridSquare = function(){
  ...
  for (i = 0; i < l; ++i) {
    ...
    console.log("test " + i + ": " + testData[i][0] + " " + JSON.stringify(testData[i][1]) +
                ...
                " expected= " + testData[i][2] + " passed = " + thisPassed);
